...
export const staffVerifyLoginOtpApi = async payload => {
  const response = await axios.post(
    `${API_BASE_URL}staff/login/verify-otp`,
    payload,
...
export const staffResendLoginOtpApi = async payload => {
  const response = await axios.post(
    `${API_BASE_URL}staff/login/resend-otp`,
    payload,
    { withCredentials: true }
  )
